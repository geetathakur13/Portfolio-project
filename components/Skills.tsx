      ...
      <div className="container mx-auto px-6">
        ...
        <div className="relative max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <h3 className="text-3xl font-bold text-gray-800 mb-4">Learning Timeline</h3>
          ...
          <div className="space-y-12">
            {skillsData.map((item, index) => (
              ...
              >
                {/* Timeline Dot */}
                <div className="absolute left-1/2 transform -translate-x-1/2 w-6 h-6 bg-white border-4 border-blue-500 rounded-full z-10 hidden lg:block"></div>
                ...
                >
                  <div className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-100">
                    <div className={`h-2 bg-gradient-to-r ${item.color} rounded-t-xl`}></div>
                    <div className="p-6">
                      <div className="flex items-center justify-between mb-4">
                        <span className={`text-2xl font-bold bg-gradient-to-r ${item.color} bg-clip-text text-transparent`}>
                          {item.year}
                        </span>
                        <div className="lg:hidden w-4 h-4 bg-blue-500 rounded-full"></div>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {item.skills.map((skill, skillIndex) => (
                          <span
                            key={skillIndex}
                            className={`px-3 py-1 text-sm font-medium text-white bg-gradient-to-r ${item.color} rounded-full`}
                          >
                            {skill}
                          </span>
                        ))}
